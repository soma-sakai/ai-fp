...
const BalanceTable: React.FC<BalanceTableProps> = ({ simulationData }) => {
  ...
  const sortedData = [...simulationData].sort((a, b) => {
    ...
    if (sortKey.includes('.')) {
      ...
      // 型安全な方法でネストしたプロパティにアクセス
      if (parent === 'expenses' && a.expenses && b.expenses) {
        aValue = a.expenses[child] || 0;
        bValue = b.expenses[child] || 0;
      } else if (parent === 'investment' && a.investment && b.investment) {
        aValue = (a.investment as any)[child] || 0;
        bValue = (b.investment as any)[child] || 0;
      } else if (parent === 'mortgage' && a.mortgage && b.mortgage) {
        aValue = (a.mortgage as any)[child] || 0;
        bValue = (b.mortgage as any)[child] || 0;
      } else {
        aValue = 0;
        bValue = 0;
      }
    } else {
      aValue = (a as any)[sortKey] || 0;
      bValue = (b as any)[sortKey] || 0;
    }
